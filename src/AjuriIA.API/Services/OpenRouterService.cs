...
/// <summary>
/// Provider OpenRouter usado como fallback quando os modelos Gemini não respondem.
/// </summary>
    ...
    string model) : ILLMService
{
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(options.SiteUrl))
            request.Headers.TryAddWithoutValidation("HTTP-Referer", options.SiteUrl);
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            logger.LogWarning(
                "[OpenRouter] modelo {Model} retornou {Status}: {Body}",
                model,
                (int)response.StatusCode,
                errorBody);
            throw new HttpRequestException(
                $"OpenRouter {(int)response.StatusCode} ({response.StatusCode}) no modelo {model}: {errorBody}",
                inner: null,
        ...
        while ((line = await reader.ReadLineAsync(ct)) is not null)
        {
            ...
            if (root.TryGetProperty("error", out var error))
            {
                var errorMsg = error.TryGetProperty("message", out var msg)
                    ? msg.GetString() ?? "OpenRouter API error"
                    : "OpenRouter API error";
                throw new HttpRequestException($"OpenRouter API error no modelo {model}: {errorMsg}");
            }
